...
Read [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) before changing runtime messages, browser API wrappers, script loading order or security boundaries. Use [`docs/DEVELOPMENT.md`](../docs/DEVELOPMENT.md) to find the correct files for common feature and maintenance tasks. Shared runtime message names belong in `src/shared/messages.js`; Chrome/Firefox compatibility wrappers belong in `src/shared/browser-api.js`.

Content-page changes must respect the current module boundaries: repository parsing in `src/content/repository-context.js`, GitHub DOM knowledge in `src/content/github-dom.js`, mount selection in `src/content/placement.js`, page state in `src/content/state.js`, trusted HTML requests in `src/content/page-client.js`, release parsing/loading in `src/content/release/`, navigation/observer ownership in `src/content/lifecycle.js`, trusted user actions in `src/content/actions.js`, root placement workflows in `src/content/mount-controller.js`, platform classification in `src/content/platform.js`, and reusable presentation in `src/content/ui/`. `src/content.js` is only the composition root. Component styles belong in `src/styles/`; do not recreate a monolithic `styles.css` or add duplicate route parsers, GitHub selector sets, page fetchers, release caches or observers.
...
Runtime file order and size budgets are defined in `scripts/project-structure.mjs`. Add new modules there instead of extending command strings in `package.json`. `npm run check:architecture` rejects oversized handwritten files, duplicate or unresolved `GHDN*` APIs, raw runtime-message literals, shared-to-runtime dependencies, cross-context dependencies, module cycles and manifest/HTML load-order drift.

Node tests are discovered automatically from `tests/**/*.test.js` and `tests/**/*.test.mjs`. A new test file does not require a `package.json` edit.
